...
interface TermsAndCounditionsProps {
  isVisible: boolean;
  onClose: () => void;
}

const TermsAndCounditions: React.FC<TermsAndCounditionsProps> = ({
  isVisible,
  onClose,
}) => {
  ...
  const loadData = async () => {
    try {
      ...
      const res = await client
        .get("/api/v1/appcontent/terms-and-conditions")
        .send<{ data: string }>();
      setData(res.data);
      setLoading(false);
    } catch (error) {
...
export default TermsAndCounditions;
