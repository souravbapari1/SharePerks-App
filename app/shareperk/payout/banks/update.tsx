...
import {
  ...
  getBanks,
  updateBank,
} from "../../../../network/worker/bank";
import { useAppDispatch } from "../../../../redux/hooks";
import { setUserBanksData } from "../../../../redux/slice/userSlice";

function UpdateBank() {
  const pageData = useLocalSearchParams<{
    name: string;
    number: string;
    ifsc: string;
    id: string;
  ...
  const savebank = async () => {
    try {
      setLoading(true);
      const response = await updateBank(pageData.id, {
      ...
      Toast.show({
        ...
        text2: "Error update your bank details",
        type: "error",
      });
    }
  };
  ...
  const showConfirmDelete = () => {
    Alert.alert(
      "Confirm You Delete", // Title of the alert
      "Are you sure you want to delete this account?", // Message
      [
        {
          text: "Cancel",
          onPress: () => console.log("Action canceled"), // On cancel, log this message or perform any action
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              await deleteBank(pageData.id);
              const banks = await getBanks();
              dispatch(setUserBanksData(banks.banks));
              setLoading(false);
              ...
              Toast.show({
                ...toastConfig,
                text1: "Something went wrong!",
                text2: "Error delete your bank details",
                type: "error",
              });
            }
          },
  ...
  return (
    <GestureHandlerRootView className="flex-1">
      <HeaderAppBar title="Update Bank">
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
          ...
          <View className="mt-10">
            ...
            <Pressable
              className="authBtn bg-red-500/80 mt-5"
              disabled={loading}
            ...
            >
              {loading ? (
                ...
                <Text className="text-white">Delete Bank Account</Text>
              )}
            </Pressable>
          </View>
